          ...
          <div className="absolute w-full">
            {testimonials.map((testimonial, index) => (
              <div
                ...
                className={`absolute top-0 left-0 w-full transform-style-3d transition-all duration-700 ${
                  index === activeIndex
                    ? "opacity-100 translate-z-0 rotate-y-0"
                    : index === (activeIndex + 1) % testimonials.length
                    ? "opacity-50 translate-z-[-100px] rotate-y-40 translate-x-[50%]"
                    : "opacity-50 translate-z-[-100px] rotate-y-[-40deg] translate-x-[-50%]"
                }`}
              >
                <div className="p-8 rounded-2xl bg-white dark:bg-gray-800 shadow-lg hover:shadow-xl transition-all duration-300">
